...
void atender_cliente(int connfd)
{
	int n;
	char buf[MAXLINE] = {0};
	char salida[SHA256_BLOCK_SIZE]={0};
	...
	while(1){
		...
		sha256_init(&sha);
                sha256_update(&sha,(BYTE*)buf,strlen(buf)-1);
                sha256_final(&sha,(BYTE*)hash);
		sprint_hex(salida,hash,SHA256_BLOCK_SIZE);
